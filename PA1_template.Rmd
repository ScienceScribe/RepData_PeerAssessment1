...
```
The mean step number per day is `r mean` steps. The median for the step number per day is `r median` steps.
...
```
The 5-minute interval that contains the maximum average number of steps is at `r maxInterval` minutes from midnight.
...
```
The total number of rows with missing values is `r nrowsincomplete`.
...
```
For the filled-in data, the mean step number per day is `r meanFilled` steps and the median for the step number per day is `r medianFilled` steps. The means for the filled-in data and non-filled-in data are identical and the medians differ by only 1 step. Thus, imputing missing data did not have much impact on the estimates of the total step number per day.
